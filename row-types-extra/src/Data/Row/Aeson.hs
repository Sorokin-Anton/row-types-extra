...
-- All credits to row-types maintainers, file is copypasted from this repo, but they do not move this file to package, so it is located here for now
-----------------------------------------------------------------------------
-- |
...
instance Forall r ToJSON => ToJSON (Var r) where
  toJSON v = object [foo l]
    where (l, foo) = Var.eraseWithLabels @ToJSON (\a b -> b .= a) v
